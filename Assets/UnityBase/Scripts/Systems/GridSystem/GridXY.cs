...
{
    public class GridXY<T> where T : new()
    {
        private int _width;
        private int _height;
        ...
        public GridXY(int width, int height, float cellSize, Vector3 originPos, Transform parent, Func<GridXY<T>, int, int, T> createGridObject)
        {
            _width = width;
            _height = height;
            _cellSize = cellSize;
        ...
        private void SetDefaultGridData(Func<GridXY<T>, int, int, T> createGridObject)
        {
        ...
        public Vector3 GetWorldPosition(int x, int y) => new Vector3(x, y) * _cellSize + _originPos;
        ...
        public T GetGridObject(int x, int y) => !IsInRange(x, y) ? default : _gridArray[x, y];
        ...
        public bool IsInRange(int x, int y) => x >= 0 && y >= 0 && x < _width && y < _height;
